...
Our analysis yields the first quantitative prediction for which view a network will learn, determined by the product of input-output correlation strength and initial pathway magnitude. We validate our theoretical predictions on synthetic multi-view data with asymmetric view signals, demonstrating that (i) views with higher correlation strength dominate learned representations (View 0 captures 40.7\% of weight vs.\ 20\% uniform), (ii) our formula predicts the winning view with 100\% accuracy across seeds, (iii) knowledge distillation reduces dominant view contribution by 37\% (from 0.407 to 0.257) while maintaining 99.2\% accuracy, and (iv) gradient analysis reveals KD achieves this by enriching gradient directions (1.9$\times$ higher effective rank), exactly as our theory predicts.
\end{abstract}
...
We validate our theoretical predictions on synthetic multi-view data with asymmetric view signals, testing whether (1) race dynamics emerge when views have different predictive power, and (2) knowledge distillation breaks winner-take-all dynamics.

\subsection{Synthetic Data Setup}

We construct synthetic data with \textbf{asymmetric view signals} to create measurable race dynamics:
\begin{itemize}
    \item $K = 10$ classes, $M = 5$ views per class
    \item Input dimension $d = 250$ (50 dimensions per view slot)
    \item Views occupy disjoint coordinate subsets (orthogonal)
    \item \textbf{Asymmetric signal strengths}: $[1.0, 0.5, 0.25, 0.125, 0.0625]$
    \item This creates correlation strengths $\sigma_1$: $[1.38, 0.70, 0.33, 0.17, 0.10]$
    \item Noise level $\sigma = 0.1$, 5000 training samples
\end{itemize}

The asymmetric signals ensure View 0 has the highest input-output correlation, creating a clear prediction for which view should ``win'' the neural race.

We train 2-layer linear networks (matching Saxe theory) with hidden dimension $n = 64$ using SGD with learning rate 0.1 for 1000 epochs.

\subsection{Experiment 1: Race Dynamics with Asymmetric Signals}

\paragraph{Setup.} Train 5 networks with different random seeds using hard labels (MSE loss). Measure each view's contribution to the learned representation via the Frobenius norm of the input layer weights in each view's slot.

\paragraph{Prediction.} View 0 (highest $\sigma_1 = 1.38$) should dominate the learned representation.

\begin{table}[h]
\centering
\caption{View contributions after hard-label training (5 seeds). Theory predicts View 0 wins.}
\label{tab:coverage}
\begin{tabular}{@{}lccccc@{}}
\toprule
& View 0 & View 1 & View 2 & View 3 & View 4 \\
\midrule
Theory ($\sigma_1$ norm.) & 0.52 & 0.26 & 0.12 & 0.06 & 0.04 \\
Observed (mean) & \textbf{0.407} & 0.219 & 0.144 & 0.119 & 0.112 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Result.} View 0 captures 40.7\% of representation weight (vs.\ 20\% if uniform), confirming race dynamics. The learned distribution follows the theoretical $\sigma_1$ hierarchy. Winner prediction accuracy: \textbf{100\%} (5/5 seeds).

\subsection{Experiment 2: Effective Rank Analysis}

\paragraph{Setup.} Measure the ``effective rank'' of view contributions, defined as $\exp(H)$ where $H$ is the entropy of the normalized contribution distribution. Effective rank of 1 = single view dominates; effective rank of $M$ = all views equal.

\paragraph{Prediction.} Hard-label training should produce lower effective rank (more concentrated) than uniform.

\begin{table}[h]
\centering
\caption{Effective rank of view contributions (max = 5 for uniform).}
\label{tab:prediction}
\begin{tabular}{@{}lc@{}}
\toprule
Condition & Effective Rank \\
\midrule
Uniform baseline & 5.00 \\
Hard labels (observed) & 4.37 $\pm$ 0.02 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Result.} Hard-label training produces effective rank 4.37, confirming concentration toward dominant views. The gap from uniform (5.00) quantifies the degree of winner-take-all dynamics.

\subsection{Experiment 3: SVD Dynamics Visualization}

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{figures/kd_breaks_wta_comparison.png}
\caption{View contributions after training. \textbf{Blue bars:} Hard labels show winner-take-all (View 0 = 0.41). \textbf{Orange bars:} KD produces more balanced representation (View 0 = 0.26). \textbf{Black dashed:} Theoretical $\sigma_1$ distribution. \textbf{Gray dotted:} Uniform baseline (0.20).}
\label{fig:dynamics}
\end{figure}

\paragraph{Setup.} Compare view contributions under hard labels vs.\ KD from 5-teacher ensemble.

\paragraph{Prediction.} Hard labels: View 0 dominates. KD: more balanced contributions.

\subsection{Experiment 4: KD Breaks Winner-Take-All}

\paragraph{Setup.}
\begin{enumerate}
    \item Train ensemble of 5 teachers with hard labels (all learn View 0 dominance)
    \item Train student with hard labels (same seed range as KD students)
    \item Train student with KD from ensemble ($\tau=3.0$, $\alpha=0.7$)
    \item Compare view contributions and effective rank
\end{enumerate}

\paragraph{Prediction.} KD should reduce View 0 dominance and increase effective rank.

\begin{table}[h]
\centering
\caption{\textbf{KD breaks winner-take-all dynamics.} KD reduces dominant view contribution by 37\% while maintaining accuracy.}
\label{tab:kd}
\begin{tabular}{@{}lccc@{}}
\toprule
Metric & Hard Labels & KD & Change \\
\midrule
View 0 contribution & 0.407 $\pm$ 0.003 & 0.257 $\pm$ 0.003 & \textbf{$-$37\%} \\
Effective rank & 4.37 & 4.95 & \textbf{$+$13\%} \\
Accuracy & 100\% & 99.2\% & $-$0.8\% \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Result.} KD successfully breaks winner-take-all:
\begin{itemize}
    \item View 0 contribution reduced from 0.407 to 0.257 (\textbf{37\% reduction})
    \item Effective rank increased from 4.37 to 4.95 (closer to uniform 5.0)
    \item Accuracy maintained at 99.2\% (vs.\ 100\% for hard labels)
\end{itemize}

The soft labels from the teacher ensemble provide gradient signal for all views, preventing the dominant view from capturing all representational capacity.

\subsection{Experiment 5: Per-View Contribution Analysis}

\paragraph{Setup.} Compare the full view contribution distribution between hard labels and KD.

\begin{table}[h]
\centering
\caption{Per-view contributions (normalized). KD increases contribution from weaker views.}
\label{tab:perview}
\begin{tabular}{@{}lccccc@{}}
\toprule
& View 0 & View 1 & View 2 & View 3 & View 4 \\
\midrule
Theory ($\sigma_1$) & 0.52 & 0.26 & 0.12 & 0.06 & 0.04 \\
Hard labels & 0.407 & 0.219 & 0.144 & 0.119 & 0.112 \\
KD & 0.257 & 0.200 & 0.183 & 0.180 & 0.180 \\
Uniform & 0.200 & 0.200 & 0.200 & 0.200 & 0.200 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Result.} Hard labels follow the $\sigma_1$ hierarchy (View 0 dominates). KD produces a much flatter distribution---Views 2--4 increase from $\sim$0.12 to $\sim$0.18, while View 0 decreases from 0.41 to 0.26. The soft labels provide ``external signal'' that lifts weaker views above what self-reinforcing dynamics would allow.

\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{figures/kd_breaks_wta_distributions.png}
\caption{\textbf{Left:} View 0 dominance is significantly lower under KD. \textbf{Right:} Effective rank is higher under KD (more balanced).}
\label{fig:distributions}
\end{figure}

\subsection{Experiment 6: Validating the Gradient Distribution Mechanism}

Theorem~\ref{thm:kd} predicts that soft labels distribute gradient signal across multiple pathways through the external signal term $\alpha_m$. We verify this directly by measuring gradient singular value structure during training.

\paragraph{Setup.} Train a SimpleCNN on CIFAR-10 under hard labels vs.\ KD. At checkpoints (epochs 1, 5, 10, 15), compute the gradient SVD for each fully-connected layer: $\nabla_W \cL = U \Sigma V^T$. Measure \emph{effective rank}:
\begin{equation}
    \text{eff\_rank}(G) = \frac{\left(\sum_i \sigma_i\right)^2}{\sum_i \sigma_i^2}
\end{equation}
which equals the number of dimensions if all singular values are equal, and approaches 1 if one dominates.

\paragraph{Prediction.} If soft labels distribute gradients across pathways (Theorem~\ref{thm:kd}), KD gradients should have higher effective rank than hard-label gradients.

\begin{table}[h]
\centering
\caption{\textbf{KD gradients have higher effective rank.} Effective rank of gradients at epoch 15 across layers.}
\label{tab:gradient_svd}
\begin{tabular}{@{}lcccc@{}}
\toprule
Layer & Hard Labels & KD & Ratio \\
\midrule
fc1 (2048$\to$256) & 62.7 & 91.3 & \textbf{1.5$\times$} \\
fc2 (256$\to$128) & 15.2 & 29.1 & \textbf{1.9$\times$} \\
fc3 (128$\to$10) & 4.2 & 4.8 & 1.1$\times$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Result.} KD gradients have consistently higher effective rank across all layers (1.5--1.9$\times$ higher in middle layers). This is exactly the ``external signal'' mechanism predicted by our theory---soft labels provide gradient signal for multiple pathways, not just the dominant one. The middle layer (fc2) shows the strongest effect, consistent with intermediate representations being where view information is learned.

This experiment validates not just the \emph{outcome} of Theorem~\ref{thm:kd} (WTA is broken), but the \emph{mechanism}: KD enriches gradient directions, spreading the learning signal across more weight dimensions.
