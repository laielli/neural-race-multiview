...
Our analysis yields the first quantitative prediction for which view a network will learn, determined by the product of input-output correlation strength and initial pathway magnitude. We validate our theoretical predictions on synthetic multi-view data with asymmetric view signals, demonstrating that (i) views with higher correlation strength dominate learned representations (View 0 captures 40.7\% of weight vs.\ 20\% uniform), (ii) our formula predicts the winning view with 100\% accuracy across seeds, and (iii) knowledge distillation reduces dominant view contribution by 37\% (from 0.407 to 0.257) while maintaining 99.2\% accuracy, breaking winner-take-all dynamics.
\end{abstract}
