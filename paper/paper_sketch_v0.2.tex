...
\caption{\textbf{Overview of three core findings.} (Left) Finding 1: All teachers converge to identical representations---random seeds don't create diversity. (Center) Finding 2: Hierarchical structure enables 5.3$\times$ speedup. (Right) Finding 3: Balance-speed trade-off---achieving higher effective rank requires sacrificing learning speed.}
\label{fig:overview}
\end{figure}
...
\caption{\textbf{Finding 1: All CE teachers converge to identical representations.} (a) View 0 contribution is nearly identical across all 5 teachers (std = 0.001). (b) Effective rank is also identical. Random seeds do not create diversity in our GDLN setting.}
\label{fig:teacher_homogeneity}
\end{figure}

The standard deviation of View 0 contribution across teachers is \textbf{0.001}---effectively zero (Figure~\ref{fig:teacher_homogeneity}). All teachers learn the same WTA representation favoring View 0.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig4_kd_transfer_mechanism.png}
\caption{\textbf{KD transfers teacher representation to student.} The KD student's view contributions closely match the teacher average, not the hard-label baseline. The student inherits the teacher's V0 bias.}
\label{fig:transfer}
\end{figure}
...
\caption{\textbf{Finding 2: Hierarchical structure enables 5.3$\times$ speedup.} (a) Learning curves showing KD reaches 90\% accuracy much faster. (b) Direct comparison: KD reaches 90\% in 30 epochs vs.\ 160 for hard labels.}
\label{fig:hierarchical}
\end{figure}

KD achieves 90\% accuracy in \textbf{30 epochs} vs.\ 160 for hard labels---a \textbf{5.3$\times$ speedup} (Figure~\ref{fig:hierarchical}).
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig6_soft_label_information.png}
\caption{\textbf{Soft labels encode inter-class similarity.} (a) Soft label entropy distribution shows meaningful spread (mean 1.59 vs.\ max 2.30), not peaked like hard labels. (b) The teacher places 17.7\% probability mass on same-superclass alternatives, encoding ``this dog looks like a wolf.''}
\label{fig:soft_labels}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{figures/fig5_forced_diversity.png}
\caption{\textbf{Forced diversity via single-view teachers.} Each teacher only sees one view (others masked). The resulting view contribution matrix shows diagonal dominance: each teacher has highest contribution on its designated view.}
\label{fig:forced_diversity}
\end{figure}

Each teacher has highest contribution on its designated view (Figure~\ref{fig:forced_diversity}). \textbf{Diagonal dominance verified.}
...
\caption{\textbf{Finding 3: Fundamental balance-speed trade-off.} (a) Scatter plot showing the Pareto frontier: uniform weighting is fast but WTA; inverse weighting achieves best balance but is slowest. (b) Bar comparison of effective rank (green, higher=better) and epochs to 90\% (red, lower=better).}
\label{fig:tradeoff}
\end{figure}
...
Figure~\ref{fig:tradeoff} illustrates this fundamental trade-off.
